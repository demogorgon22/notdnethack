...
boolean greatequip;
{
	...
	if(mm == PM_ELDER_PRIEST){
		...
		otmp = mksobj(PLATE_MAIL, MKOBJ_ARTIF);
		...
		if(rn2(2))
			...
			otmp = mksobj(RUNESWORD, MKOBJ_ARTIF);
...
// #endif /* CONVICT */
		...
		case PM_SIR_GARLAND:
			...
			(void) mongets(mtmp, PLATE_MAIL, mkobjflags);
			(void) mongets(mtmp, find_pcloth(), mkobjflags);
		break;
		case PM_GARLAND:
...
#undef NazPastArmor
		...
		switch (mm) {
		    ...
		    case PM_EFREET:
				...
				set_material_gm(otmp, OBSIDIAN_MT);
				(void) mpickobj(mtmp, otmp);
				otmp = mksobj(RING_MAIL, mkobjflags);
				set_material_gm(otmp, OBSIDIAN_MT);
				(void) mpickobj(mtmp, otmp);
				(void)mongets(mtmp, SHEMAGH, mkobjflags);
			break;
		    ...
		    case PM_EFREET_MERCHANT:
				...
				set_material_gm(otmp, OBSIDIAN_MT);
				(void) mpickobj(mtmp, otmp);
				otmp = mksobj(SPLINT_MAIL, mkobjflags);
				set_material_gm(otmp, OBSIDIAN_MT);
				(void) mpickobj(mtmp, otmp);
				(void)mongets(mtmp, SHEMAGH, mkobjflags);
			break;
...
//				(void) mpickobj(mtmp,otmp);
	...
	switch(ptr->mtyp){
		case PM_AHAZU:
			(void)mongets(mtmp, FOOD_RATION, mkobjflags);
			(void)mongets(mtmp, FOOD_RATION, mkobjflags);
			(void)mongets(mtmp, FOOD_RATION, mkobjflags);
			(void)mongets(mtmp, FOOD_RATION, mkobjflags);
			break;
		...
		case PM_ANDREALPHUS:
			(void)mongets(mtmp, SENSOR_PACK, mkobjflags);
			(void)mongets(mtmp, CAN_OF_GREASE, mkobjflags);
			break;
		case PM_ANDROMALIUS:
			(void)mongets(mtmp, SACK, mkobjflags);
			m_initthrow(mtmp, DAGGER, 12, mkobjflags);
			break;
		case PM_ASTAROTH:
			(void)mongets(mtmp, UPGRADE_KIT, mkobjflags);
			(void)mongets(mtmp, UPGRADE_KIT, mkobjflags);
			break;
		case PM_BALAM:
			(void)mongets(mtmp, DAGGER, mkobjflags);
		...
		case PM_BERITH:{
			(void)mongets(mtmp, PLATE_MAIL, mkobjflags);
			(void)mongets(mtmp, LONG_SWORD, mkobjflags);
			struct obj *otmp = mksobj(HELMET, mkobjflags); //make articrown (ok made crown of antisucc)
		...
		case PM_BUER:{
			struct obj *otmp = mksobj(AMULET_OF_LIFE_SAVING, mkobjflags); //super healing thing (ok made amulet of not dying or stat screws and regen)
		...
		case PM_DANTALION:{
			...
			(void) mongets(mtmp, GLOVES, mkobjflags);
			(void) mongets(mtmp, HIGH_BOOTS, mkobjflags);
			(void) mongets(mtmp, HELM_OF_TELEPATHY, mkobjflags);
			break;
		}
		case PM_SHIRO: {
			struct obj *otmp;
		...
		case PM_ECHIDNA:
			(void) mongets(mtmp, rnd_misc_item(mtmp), mkobjflags);//give her some junk cause why not?
			(void) mongets(mtmp, rnd_misc_item(mtmp), mkobjflags);
			(void) mongets(mtmp, rnd_misc_item(mtmp), mkobjflags);
			break;
		...
		case PM_EURYNOME:{
			struct obj *otmp;
			otmp = mksobj(SPEED_BOOTS, mkobjflags);
		...
		case PM_JACK:{
			struct obj *otmp;
			otmp = mksobj(POT_BOOZE, mkobjflags);
			otmp->blessed = FALSE;
			otmp->cursed = TRUE;
			(void) mpickobj(mtmp, otmp);
			(void) mongets(mtmp, APPLE, mkobjflags);
		...
		case PM_MARIONETTE:{
			struct obj *otmp;
			otmp = mksobj(GRAPPLING_HOOK, mkobjflags);
		...
		case PM_NABERIUS:
			for(int i=0;i<8+rnd(3);i++)	
				(void) mongets(mtmp, POT_BOOZE, mkobjflags);
			break;
		case PM_ORTHOS:{
			struct obj *otmp;
			otmp = mksobj(CLOAK_OF_DISPLACEMENT, mkobjflags);
		...
		case PM_PAIMON:{
			struct obj *otmp;
			otmp = mksobj(HELM_OF_BRILLIANCE, mkobjflags);
			otmp->spe = 3;
			set_material_gm(otmp, GEMSTONE);
			(void) mpickobj(mtmp, otmp);
			otmp = mksobj(QUARTERSTAFF, mkobjflags);
		...
		case PM_YMIR:{
			...
			otmp->spe = 7;
			(void) mpickobj(mtmp, otmp);
			otmp = mksobj(CRYSTAL_HELM, mkobjflags);
			otmp->blessed = FALSE;
			otmp->cursed = FALSE;
			otmp->objsize = MZ_GIGANTIC;
			fix_object(otmp);
			otmp->spe = 7;
			(void) mpickobj(mtmp, otmp);
			otmp = mksobj(CRYSTAL_BOOTS, mkobjflags);
			otmp->blessed = FALSE;
			otmp->cursed = FALSE;
			otmp->objsize = MZ_GIGANTIC;
			fix_object(otmp);
			...
			otmp->spe = 10;
			(void) mpickobj(mtmp, otmp);
			otmp = mksobj(CRYSTAL_SHIELD, mkobjflags);
			otmp->blessed = FALSE;
			otmp->cursed = FALSE;
			otmp->objsize = MZ_GIGANTIC;
			fix_object(otmp);
