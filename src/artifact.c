	...
	int hypothetical;
{
    ...
    if (yours) {
		...
		if(!hypothetical){
			if(obj->oartifact == ART_ROD_OF_SEVEN_PARTS && u.inherited != ART_ROD_OF_SEVEN_PARTS)
				give_law_trophy();
		...
		switch(illithidFunc){
			...
			case COMMAND_ELDER:
				if(Is_astralevel(&u.uz) && a_align(u.ux,u.uy) == A_VOID && u.uhave.amulet){
					...
					schedule_goto(&nearvoid_level, FALSE, FALSE, 0, (char *)0, (char *)0, 0, 0);	
...
struct obj *obj;
{
	...
	if(obj->ovar1 < monstermoves){	
		...
		add_menu(tmpwin, NO_GLYPH, &any,
			'c', 0, ATR_NONE, buf,
			MENU_UNSELECTED);
		if(obj->cobj && obj->cobj->oartifact == ART_ELDER_CEREBRAL_FLUID){//check if elder cerebral fluid is in the illithid
