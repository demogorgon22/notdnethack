...
register struct monst *mtmp;
{
	...
	if(has_template(mtmp, CRYSTALFIED)){
		...
		goto default_1;
	    case PM_EDEN:
			obj = mksobj_at(SILVER_DRAGON_SCALES, x, y, MKOBJ_NOINIT);
			obj = oname(obj, artiname(ART_EDEN_S_SCALES));
			newsym(x,y);
		goto default_1;
	    case PM_HUGINN:
			obj = mksobj_at(DAGGER, x, y, MKOBJ_NOINIT);
		...
		goto default_1;
	    case PM_SIMURGH:
			obj = mksobj_at(FEATHER, x, y, MKOBJ_NOINIT);
...
//		goto default_1;
		...
		case PM_DEATH_EYE:
			{
				otmp = oname(mksobj(EYEBALL, NO_MKOBJ_FLAGS),
	    ...
	    case PM_MARIONETTE:
			num = d(2,4);
			while(num--)
				obj = mksobj_at(GRAPPLING_HOOK, x, y, NO_MKOBJ_FLAGS);
			rem_mx(mtmp, MX_ENAM);
		break;
	    case PM_GOLD_GOLEM:
...
boolean was_swallowed;			/* digestion */
{
	...
	for(i = 0; i < NATTK; i++) {
		...
		if (aatyp == AT_BOOM  &&  adtyp != AD_HLBD && adtyp != AD_POSN) {
			...
			if(adtyp == AD_JAILER){
				...
				explode_pa(mon->mx, mon->my, AD_NUKE, MON_EXPLODE, tmp, EXPL_MUDDY, 2, mdat);
			} else if(mdat->mattk[i].adtyp == AD_JACK){
				explode_pa(mon->mx, mon->my, AD_FIRE, MON_EXPLODE, tmp, EXPL_FIERY, 5, mdat);
				return TRUE;
	 ...
	 */
	if (has_template(mon, SKELIFIED) || has_template(mon, SLIME_REMNANT) || has_template(mon, WHISPERING))
		return FALSE;
...
cleanup:
	...
	} else if (mtmp->mtame && u.ualign.type != A_VOID && !banish_kill_mon(mtmp) && !(EDOG(mtmp) && EDOG(mtmp)->dominated) && !Role_if(PM_ANACHRONOUNBINDER) ) {
		adjalign(-15);	/* bad!! */
