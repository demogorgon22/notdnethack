	...
	case AD_ALIG:
		if(youdef){
			...
			if(uarmh && uarmh->otyp == HELM_OF_OPPOSITE_ALIGNMENT) {
				...
				struct obj *otmp;
				otmp = mksobj(HELM_OF_OPPOSITE_ALIGNMENT, NO_MKOBJ_FLAGS);		
				curse(otmp);
				otmp->spe = -6;
...
boolean verbose;
{
	...
	if(mon == &youmonst){
		...
		if (has_spear_point(MON_WEP(mon),GREEN_FLUORITE)) {
                	if (visible && verbose) You(mons_item_mal_aura, s_suffix(mon_nam(mon)), "green fluorite spear point");
                	return TRUE;
        	}
		if (MON_WEP(mon) &&
...
expl_common:
			...
			else if (attk->adtyp == AD_COLD || attk->adtyp == AD_ECLD || attk->adtyp == AD_UHCD){
				if (!UseInvCold_res(mdef)){
					if (mlev(magr) > rn2(20))
						destroy_item(mdef, POTION_CLASS, AD_COLD);
				}
...
#define eld_bon_size ((u.ueldritch_style == SPE_FINGER_OF_DEATH) ? 13 : (\
	...
	if (valid_weapon_attack || unarmed_punch || unarmed_kick || unarmed_butt)
	{
		...
		/*Blind, confused (spun around), players attacking lokoban pinata with a bashing weapon one shot it*/
		if(youagr && Blind && Confusion && weapon && valid_weapon_attack && (attack_mask(weapon, 0, 0, magr) & WHACK) && mdef && monsndx(mdef->data) == PM_LOKOBAN_PINATA)
			bonsdmg += mdef->mhp*4;
