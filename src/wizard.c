...
u_recognize_stars()
{
	...
	if(u.uwizard || u.upriest || Role_if(PM_RANGER))
		return (RACE_IF_DROW || Race_if(PM_DWARF) || Race_if(PM_GNOME)) ? ACURR(A_INT) > 18 : ACURR(A_INT) > 13;
	// Anas
	if(Role_if(PM_ANACHRONONAUT) || Role_if(PM_UNDEAD_HUNTER))
		return ACURR(A_INT) > 13;

	// Catch all
	return (RACE_IF_DROW || Race_if(PM_DWARF) || Race_if(PM_GNOME)) ? FALSE : ACURR(A_INT) > 15;
}
