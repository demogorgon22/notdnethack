...
#define chrgd			(!obj || obj->ovar1_charges>0)
	...
	case LAVA_BALL:			if (obj&&obj->ovar1_acidSplashDamage){ ocn = 0; flat = obj->ovar1; } else{ add(6); } break;
...
boolean youagr;
{
	return ((otmp->oclass == WEAPON_CLASS || is_weptool(otmp)
			...
			&& (!youagr || otmp->owt <= max(10, (P_SKILL(P_TWO_WEAPON_COMBAT) + 1)*10))// not too heavy
