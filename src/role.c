...
{	{"Anachrononaut", 0}, {
	...
	ART_ANNULUS,
	SA_HUMAN|SA_DWARF|SA_ELF|SA_VAMPIRE|SA_HALF_DRAGON|SA_CLOCKWORK|SA_SALAMANDER|SA_ETHEREALOID|SA_ENT|SA_GNOME, ROLE_MALE|ROLE_FEMALE |
...
{	{"Illithanachronounbinder", 		 0}, {
	...
	GOD_ILSENSINE, GOD_RESISTANCE, GOD_THOON,	/* ahahah */
	GOD_THOON,
	...
	SA_HUMAN|SA_DWARF|SA_GNOME|SA_ELF|SA_ORC|SA_HALF_DRAGON|SA_CHIROPTERAN|SA_SALAMANDER|SA_ENT|SA_YUKI_ONNA|SA_TIEFLING|SA_AASIMAR|SA_FORMIAN|SA_DRIDER|SA_CENTAUR, ROLE_MALE|ROLE_FEMALE |
	  ROLE_LAWFUL,
	/* Str Int Wis Dex Con Cha */
	{  12, 10,  7, 12,  12,  7 },
	{  24, 20, 15, 20, 24, 15 },
...
{	{"Tourist", 0}, {
	...
	SA_HUMAN|SA_ETHEREALOID, ROLE_MALE|ROLE_FEMALE | ROLE_NEUTRAL,
	/* Str Int Wis Dex Con Cha */
	{   7, 10,  6,  7,  7, 10 },
	{  15, 10, 10, 15, 30, 20 },
	/* Init   Lower  Higher */
...
validdescendant(int rolenum)
{
	...
	return (rolenum >= 0 && rolenum < SIZE(roles)-1 &&
		!(roles[rolenum].malenum == PM_ANACHRONONAUT
			...
			|| roles[rolenum].malenum == PM_KENSEI
		));
}
...
int newgame;
{
	...
	if(Role_if(PM_EXILE)){
		...
		urole.lgod = GOD_UTU;
		urole.ngod = GOD_KOSSUTH;
		urole.cgod = GOD_GARYX;
